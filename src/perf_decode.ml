...
let symbol_and_offset_and_dso_re = Re.Perl.re {|^(.*)\+(0x[0-9a-f]+)\s+\((.*)\)$|} |> Re.compile
...
let parse_symbol_and_offset ?perf_maps pid str ~addr : Symbol.t * int * string =
       ...
       (match Re.Group.all (Re.exec unknown_symbol_dso_re str) with
        | [| _; dso |] ->
          (* CR-someday tbrindus: ideally, we would subtract the DSO base offset
             from [offset] here. *)
          From_perf [%string "[unknown @ %{addr#Int64.Hex} (%{dso})]"], 0, dso
